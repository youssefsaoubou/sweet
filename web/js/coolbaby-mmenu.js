...
jQuery(function ($j) {
    ...
    $jmenuItems.each(function () {
        var $jthis = $j(this),
            $jdropdown = $jthis.next("ul.item-content");
        if ($jdropdown.length) {
            var pos = menuWidth;
            menuWidth += 100;
            if ($jdropdown.hasClass('content-small')) {
                $jdropdown = $j("<div style='width: 50%; float: left;' class='dropdown-small dropdown dropdown" + pos * 0.01 + "'></div>").append($jdropdown[0].outerHTML);
            }  
			
			else $jdropdown = $j("<div style='width: 50%; float: left;' class='dropdown dropdown" + pos * 0.01 + "'></div>").append($jdropdown[0].outerHTML);
